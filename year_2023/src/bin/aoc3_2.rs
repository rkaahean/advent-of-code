...
fn main() {
    ...
    for i in 0..row_len {
        for j in 0..col_len {
            ...
            let val = matrix.get(i as usize).unwrap().get(j as usize).unwrap().to_string();
            ...
            for (x, y) in DIRS {
                ...
                if (neigh_x >= 0 && neigh_x < col_len) && (neigh_y >=0 && neigh_y < row_len) {
                    let neigh_value = matrix.get(neigh_x as usize).unwrap()
                                                .get(neigh_y as usize).unwrap()
                                                .to_string();
                    ...
                    let (num, found_start, found_end) = get_num_for_idx(neigh_x as usize, neigh_y as usize, &matrix);
